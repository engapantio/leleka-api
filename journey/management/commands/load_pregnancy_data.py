    ...
    def handle(self, *args, **options):
            ...
            for item in baby_data:
                try:
                    WeekData.objects.create(
                        ...
                        baby_image=item['image'],
                        baby_size=item['babySize'],
                        baby_weight=item['babyWeight'],
                        baby_analogy=item.get('analogy', ''),
                        baby_activity=item['babyActivity'],
                        baby_development=item['babyDevelopment'],
                        interesting_fact=item['interestingFact'],
                        mom_daily_tips=item.get('momDailyTips', []),
                        days_to_childbirth=max(0, 280 - (item['weekNumber'] * 7)),
                    )
                    loaded_weeks += 1
        ...
        if os.path.exists(mom_file) and WeekData.objects.exists():
            ...
            for item in mom_data:
                try:
                    week = WeekData.objects.get(week_number=item['weekNumber'])
                    
                    # Feelings
                    for state in item['feelings']['states']:
                        MomFeeling.objects.get_or_create(week=week, feeling_state=state)
                    
                    # Tips
                    for tip in item['comfortTips']:
                        ComfortTip.objects.get_or_create(
                            week=week, category=tip['category'], defaults={'tip': tip['tip']}
                        )
                    loaded_mom += 1
